...
router.get('/', (req, res) => {
  const apps = manifests(); const procs = procMod.getAllProcesses();
  // v1.9.0 修 P1-8：归属解析统一走 appresolve 公共模块（原本这里有第三份 appFor 拷贝）
  const resolver = appresolve.createResolver(procs);
  ...
  for (const proc of procs) { const app = resolver.resolve(proc); if (!app || !app.app_id || app.app_id === 'system') continue;
    ...
    item.processes.push({ pid:proc.pid, ppid:proc.ppid, user:proc.user, comm:proc.comm, name:proc.name, pcpu:proc.pcpu, pmem:proc.pmem, rss:proc.rss, cmdline:proc.cmdline, risk_label:rk.risk_label, kill_policy:rk.kill_policy });
